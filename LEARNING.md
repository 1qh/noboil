...
Single source of truth: `noboil.config.ts`. After editing, run `bun doc/scripts/gen-all.ts` (regen RULES.md table) and `bun script/emit-env.ts` (regen `.env`). Layout: 41xx Convex camp, 42xx STDB camp, 4300 doc. Postgres host mapping is for Convex self-hosted backing — STDB has its own commitlog and uses no Postgres.
