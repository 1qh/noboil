...
const bold = (s: string) => `\u001B[1m${s}\u001B[0m`,
  ...
  COMMANDS: Record<string, string> = {
    ...
    init: 'Create a new ohmystack project',
    sync: 'Pull and apply upstream changes'
  },
  printHelp = () => {
    console.log(`\n${bold('ohmystack')} — schema-first, zero-boilerplate fullstack\n`)
    console.log(bold('Usage:'))
    console.log('  ohmystack <command> [options]\n')
    console.log(bold('Commands:'))
    for (const [name, description] of Object.entries(COMMANDS)) console.log(`  ${name.padEnd(12)} ${dim(description)}`)
    console.log(`\nRun ${dim('ohmystack <command> --help')} for command-specific options.\n`)
  },
