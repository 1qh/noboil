...
const runWorkerRef = makeFunctionReference<'action', { prompt: string; taskId: Id<'tasks'>; threadId: string }, void>(
  ...
  enqueueRunRef = makeFunctionReference<
    'mutation',
    {
      ...
      reason: 'user_message' | 'task_completion' | 'todo_continuation'
      threadId: string
    },
...
const maybeContinueOrchestratorInline = async ({
  ctx,
  taskId
}: {
  ...
  if (!task || !task.completionReminderMessageId) return { ok: false }
...
const maybeContinueOrchestrator = internalMutation({
  args: { taskId: v.id('tasks') },
  handler: async (ctx, { taskId }) => {
    return await maybeContinueOrchestratorInline({ ctx, taskId })
  }
})
...
const listTasks = q({
  ...
  handler: async (ctx, { sessionId }) => {
    const session = await ctx.db.get(sessionId)
    if (!session || session.userId !== ctx.user._id) return []
    return await ctx.db
      .query('tasks')
...
const getOwnedTaskStatus = q({
  ...
  handler: async (ctx, { taskId }) => {
    ...
    if (!task) return null
    const session = await ctx.db.get(task.sessionId)
    if (!session || session.userId !== ctx.user._id) return null
    return task
  }
})
