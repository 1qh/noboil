...
Scanned **351 files**, **7627 declared symbols**. No `@beta`/`@alpha`/`@experimental`/`@deprecated`/`@internal` JSDoc tags found — entire public surface is **stable**.
...
**42 subpath exports** from `noboil` package.json. Use exact import paths for tree-shaking.
...
Coverage of public exports (every name reachable through `noboil/...` subpaths) by mention in `doc/content/docs/*.mdx`. **493/495 mentioned (100%).**

Undocumented (first 2 of 2):

`findStdbModuleDir`, `findStdbModuleDirDeep`
