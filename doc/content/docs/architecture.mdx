...
**244 shared files · 85 cvx-only · 61 stdb-only · 2 files with cross-backend symbol divergence.** Status: 🔴 70 unaccounted-for gap(s).

| Pair                     | shared    | cvx-only            | stdb-only           | symbol gaps | status |
| ------------------------ | --------: | ------------------- | ------------------- | ----------: | ------ |
| **lib/noboil/src**       | 84        | 46 (42 unaccounted) | 48 (26 unaccounted) | 2           | 🔴     |
...
- `lib/noboil/src/index.ts` — cvx-only: — · stdb-only: `DEFAULT_HTTP_URI`, `DEFAULT_PORT`, `DEFAULT_TOKEN_KEY`, `DEFAULT_WS_URI`, `TOKEN_COOKIE_KEY`, `wsToHttp`
