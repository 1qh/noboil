...
| --------------------------------------- | ---------------- | ----------- | ----------- |
| owned CRUD                              | `crud.ts`        | ✓ (442 loc) | ✓ (140 loc) |
| org-scoped CRUD                         | `org-crud.ts`    | ✓ (502 loc) | ✓ (400 loc) |
| child CRUD                              | `child.ts`       | ✓ (273 loc) | ✓ (164 loc) |
| singleton (1-per-user)                  | `singleton.ts`   | ✓ (76 loc)  | ✓ (114 loc) |
| cache (TTL + refresh)                   | `cache-crud.ts`  | ✓ (250 loc) | ✓ (174 loc) |
| log (append-only)                       | `log.ts`         | ✓ (391 loc) | ✓ (273 loc) |
| kv (string-keyed)                       | `kv.ts`          | ✓ (168 loc) | ✓ (162 loc) |
| quota (sliding window)                  | `quota.ts`       | ✓ (121 loc) | ✓ (124 loc) |
| org schema (membership + invites)       | `org.ts`         | ✓ (299 loc) | ✓ (566 loc) |
| org members API                         | `org-members.ts` | ✓ (138 loc) | ✓ (246 loc) |
| org invites API                         | `org-invites.ts` | ✓ (107 loc) | ✓ (399 loc) |
| org join requests                       | `org-join.ts`    | ✓ (142 loc) | ✓ (261 loc) |
| presence                                | `presence.ts`    | ✓ (88 loc)  | ✓ (142 loc) |
| file uploads                            | `file.ts`        | ✓ (416 loc) | ✓ (148 loc) |
| middleware (audit, sanitize, slow-warn) | `middleware.ts`  | ✓ (64 loc)  | ✓ (69 loc)  |
