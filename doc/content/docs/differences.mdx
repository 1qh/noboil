...
| --------------------------------------- | ---------------- | ----------- | ----------- |
| owned CRUD                              | `crud.ts`        | ✓ (435 loc) | ✓ (139 loc) |
| org-scoped CRUD                         | `org-crud.ts`    | ✓ (496 loc) | ✓ (399 loc) |
| child CRUD                              | `child.ts`       | ✓ (271 loc) | ✓ (163 loc) |
| singleton (1-per-user)                  | `singleton.ts`   | ✓ (75 loc)  | ✓ (113 loc) |
| cache (TTL + refresh)                   | `cache-crud.ts`  | ✓ (248 loc) | ✓ (173 loc) |
| log (append-only)                       | `log.ts`         | ✓ (389 loc) | ✓ (272 loc) |
| kv (string-keyed)                       | `kv.ts`          | ✓ (167 loc) | ✓ (161 loc) |
| quota (sliding window)                  | `quota.ts`       | ✓ (114 loc) | ✓ (123 loc) |
| org schema (membership + invites)       | `org.ts`         | ✓ (296 loc) | ✓ (565 loc) |
