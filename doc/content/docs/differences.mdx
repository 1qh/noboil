...
| --------------------------------------- | ---------------- | ----------- | ----------- |
| owned CRUD                              | `crud.ts`        | ✓ (437 loc) | ✓ (139 loc) |
| org-scoped CRUD                         | `org-crud.ts`    | ✓ (491 loc) | ✓ (400 loc) |
| child CRUD                              | `child.ts`       | ✓ (273 loc) | ✓ (163 loc) |
| singleton (1-per-user)                  | `singleton.ts`   | ✓ (75 loc)  | ✓ (113 loc) |
| cache (TTL + refresh)                   | `cache-crud.ts`  | ✓ (250 loc) | ✓ (173 loc) |
| log (append-only)                       | `log.ts`         | ✓ (396 loc) | ✓ (274 loc) |
| kv (string-keyed)                       | `kv.ts`          | ✓ (169 loc) | ✓ (161 loc) |
