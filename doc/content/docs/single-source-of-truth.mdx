...
noboil ships **53 generators** that own **51 marker blocks** across 7 doc files. Every block is rebuilt from source on `bun docs` and CI fails on drift via `bun docs:check`.
...
| `lib/noboil/README.md` | `PACKAGE-INFO`, `DEMO-COUNT`, `HOTKEYS`, `CLI-TABLE-TYPES`, `FACTORY-TABLE`, `DEMO-TREE` |
