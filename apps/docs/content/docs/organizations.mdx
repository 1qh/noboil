...
```tsx
import { makeOrgScoped } from '@ohmystack/convex/server'
...
import { useOrg, useActiveOrg, useMyOrgs } from '@ohmystack/spacetimedb/react'
...
```typescript
import { canEditResource } from '@ohmystack/spacetimedb/react'
...
```typescript
import { useActiveOrg } from '@ohmystack/spacetimedb/react'
...
import {
  ...
  RoleBadge,
  OfflineIndicator
} from '@ohmystack/convex/components'
```

</Tab>
<Tab value="SpacetimeDB">

Build your own org UI components using the hooks from `@ohmystack/spacetimedb/react`. The `useOrg`, `useActiveOrg`, and `useMyOrgs` hooks provide all the data you need.
