...
```tsx
import { cvFile, makeOwned } from '@ohmystack/convex/schema'
...
```tsx
import { makeOrgScoped } from '@ohmystack/convex/schema'
...
```tsx
import { useOrgQuery, useOrgMutation } from '@ohmystack/convex/react'
import { EditorsSection, PermissionGuard } from '@ohmystack/convex/components'
...
```typescript
import { schema } from '@ohmystack/spacetimedb/schema'
...
import { useList, useOrg, useOrgMutation } from '@ohmystack/spacetimedb/react'
...
```tsx
import { makeBase } from '@ohmystack/convex/schema'
...
```tsx
import { presenceTable } from '@ohmystack/convex/server'
...
```tsx
import { makePresence } from '@ohmystack/convex/server'
...
```typescript
import { ohmystackStdb, makePresence } from '@ohmystack/spacetimedb/server'
import { s } from '../t'

export default ohmystackStdb(({ table, t }) => {
...
```typescript
import { child, schema } from '@ohmystack/spacetimedb/schema'
...
import { useTable, useReducer } from 'spacetimedb/react'
import { useList, usePresence } from '@ohmystack/spacetimedb/react'
import { tables, reducers } from '@/generated/module_bindings'
...
```typescript
import { createS3UploadPresignedUrl } from '@ohmystack/spacetimedb/server'
import { NextResponse } from 'next/server'
...
import { useUpload } from '@ohmystack/spacetimedb/react'
...
```tsx
import { cvFile, makeSingleton } from '@ohmystack/convex/schema'
import { object, string, enum as zenum } from 'zod/v4'
...
Uses the `bulkRm` export from any `crud()` call. No extra backend code needed.

### Frontend

```tsx
import { useMutation } from 'convex/react'
import { useBulkMutate } from '@ohmystack/convex/react'
...
```tsx
import { useQuery } from 'convex/react'
import { useList, useOwnRows } from '@ohmystack/convex/react'
...
> Available for SpacetimeDB only. Use declaration merging to set a project-wide default error type. All `@ohmystack/spacetimedb` hooks and utilities will use your type instead of the default `Error`.

```typescript
declare module '@ohmystack/spacetimedb/server' {
  interface Register {
...
```typescript
import type { RegisteredDefaultError } from '@ohmystack/spacetimedb/server'
...
> Available for SpacetimeDB only. Convex users can use Zod's `.partial()` directly or `pickValues` from `@ohmystack/convex/zod`.
...
import { getFieldErrors } from '@ohmystack/spacetimedb/server'
import { useMut } from '@ohmystack/spacetimedb/react'
...
import { toastFieldError } from '@ohmystack/spacetimedb/react'
import { useMutation } from '@ohmystack/spacetimedb/react'
import { useReducer } from 'spacetimedb/react'
import { reducers } from '@/generated/module_bindings'
...
> Available for SpacetimeDB only. `SenderError` carries `_tag: 'SenderError'` as a const property. Use it to narrow errors in catch blocks when you need to distinguish `@ohmystack/spacetimedb` reducer errors from other thrown values.

```typescript
import { extractErrorData } from '@ohmystack/spacetimedb/server'
...
For most cases, `handleError` or `matchError` is simpler. They parse the error internally without the `_tag` check. Use `_tag` when you need to re-throw non-`@ohmystack/spacetimedb` errors or integrate with an external error boundary that inspects error shape.
