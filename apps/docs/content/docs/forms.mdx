...
```tsx
import { AutoSaveIndicator } from '@ohmystack/convex/components'
...
SpacetimeDB's ~39ms latency makes optimistic updates unnecessary in most cases. The subscription update arrives before the user notices. For higher-latency deployments, use `useOptimisticMutation` from `@ohmystack/spacetimedb/react`.
...
Lower-level utilities are available from `@ohmystack/spacetimedb/react`:

```typescript
import { buildMeta, getMeta, useForm, useFormMutation } from '@ohmystack/spacetimedb/react'
```
...
`useMut` and `useFormMutation` from `@ohmystack/spacetimedb/react` automatically parse these errors and surface them as toasts or field-level validation messages.
