...
When `bulkRm` is not available, pass `rm` as a fallback — it deletes items one at a time:
...
`ohmystackStdb()` generates `get_profile` and `upsert_profile` reducers automatically.
...
```typescript
import { warnLargeFilterSet } from '@ohmystack/spacetimedb/server'
...
If you're deploying to Maincloud and latency is higher, you can add optimistic updates later. The `useOptimisticMutation` hook from `@ohmystack/spacetimedb/react` provides a placeholder API for this.
