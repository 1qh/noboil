...
export const { addEditor, bulkRm, create, list, read, rm, update } = orgCrud(
    'wiki',
    orgScoped.wiki,
...
| Rate limiting | `rateLimit` option | Manual (`checkRateLimit` from `@ohmystack/convex/server`) |
...
Need to call an external API?
  └─ Use raw Convex action (not an ohmystack builder)
