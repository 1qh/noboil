...
`@ohmystack/convex` includes a Swift codegen CLI and ships with 8 native app examples (4 mobile + 4 desktop) that consume the same Convex backend as the web demos.
...
```bash
bunx @ohmystack/convex codegen-swift --schema packages/be/t.ts --convex packages/be/convex \
...
packages/@ohmystack/convex/    Codegen CLI (codegen-swift.ts)
```
...
```bash
bunx @ohmystack/convex codegen-swift \
  --schema convex/t.ts \
  --convex convex/ \
...
2. Re-run `bunx @ohmystack/convex codegen-swift` with the same flags
...
```bash
bunx @ohmystack/convex codegen-swift --schema convex/t.ts --convex convex/ --output Generated.swift
