---
title: Ejecting
description: Step off ohmystack's conventions and take full control of your backend, one endpoint at a time.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs'

ohmystack is designed for incremental ejection. You can replace one factory at a time while the rest of your app keeps running. You don't have to eject everything — most apps settle somewhere in the middle.
...
- You need behavior that conflicts with ohmystack's conventions (custom auth, non-standard ownership)
...
```
Full @ohmystack/convex ←──────────────────────────→ Full raw Convex

crud() for all   crud() + custom    custom only,     no @ohmystack/convex
...
The factories (`ohmystackStdb()` and `table()`) generate reducer definitions. Ejecting means writing those reducers manually instead of using the factories.
...
## Step 3: Remove @ohmystack/spacetimedb from the module

```bash
bun remove @ohmystack/spacetimedb
```
...
```typescript
import { useList, usePresence, useUpload } from '@ohmystack/spacetimedb/react'
import { zodFromTable } from '@ohmystack/spacetimedb'
```

If you want to remove `@ohmystack/spacetimedb` entirely from the client too, replace:
...
- Future ohmystack improvements (you're on your own)

## What you gain

- Full control over reducer logic
- No dependency on ohmystack's internal types
