...
bunx @noboil/spacetimedb init
```

`@noboil/spacetimedb init` auto-installs all required dependencies and creates a `tsconfig.json` with the right settings. No manual `bun add` step needed.
...
Dependencies are installed automatically by `@noboil/spacetimedb init`. To add them manually:
...
`bunx @noboil/convex init` generates `convex/t.ts` with your Zod schemas. Edit it to define your tables:
