---
title: Migration
description: Adopt ohmystack incrementally — one table at a time, without touching existing code.
---
...
```tsx
import { makeOwned } from '@ohmystack/convex/schema'
...
import { ownedTable } from '@ohmystack/convex/server'
...
convex/
  ...
  note.ts       ← @ohmystack/convex crud()
  wiki.ts       ← @ohmystack/convex orgCrud()
  setup.ts      ← @ohmystack/convex setup()
```
...
import { orgTables } from '@ohmystack/convex/server'
...
```tsx
import ohmystackConvex from '@ohmystack/convex/eslint'
import { defineConfig } from 'eslint/config'

export default defineConfig([ohmystackConvex.recommended])
```
...
```tsx
import { strictApi } from '@ohmystack/convex'
...
`@ohmystack/spacetimedb` is the SpacetimeDB successor to `@ohmystack/convex`. The mental model is similar, but the underlying system is different. This guide maps Convex concepts to their SpacetimeDB equivalents.
...
import { idToWire } from '@ohmystack/spacetimedb'
...
```typescript
import { identityEquals } from '@ohmystack/spacetimedb/server'
...
import { extractErrorData, getErrorCode } from '@ohmystack/spacetimedb'
