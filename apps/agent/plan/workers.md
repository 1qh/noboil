...
const buildTaskTerminalReminder = ({ taskId, description, status, error }) => {
  const prefix =
    status === 'failed' ? 'BACKGROUND TASK FAILED' : 'BACKGROUND TASK TIMED OUT'
  return [
    '<system-reminder>',
    `[${prefix}]`,
    `Task ID: ${taskId}`,
...
const maybeContinueOrchestrator = async ({ ctx, taskId }) => {
  ...
  const enqueued = await ctx.runMutation(
    internal.orchestrator.enqueueRunIfLatest,
    {
...
const runWorker = internalAction({
  ...
  handler: async (ctx, args) => {
    const marked = await ctx.runMutation(internal.tasks.markRunning, {
      taskId: args.taskId
    })
    if (!marked.ok) return
    ...
    const heartbeatInterval = setInterval(async () => {
      try {
        await ctx.runMutation(internal.tasks.updateHeartbeat, {
          taskId: args.taskId
        })
      } catch (_error) {}
    }, 30_000)
