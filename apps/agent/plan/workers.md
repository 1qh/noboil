...
Before writing the final assistant message and calling `completeTask`, `runWorker` re-reads the task status from DB. If the task has been marked `timed_out` or `cancelled` by a concurrent cron/mutation, the worker abandons its result and returns without writing. This prevents a timed-out worker from overwriting the terminal state or saving stale output after the cron has already cleaned up.
