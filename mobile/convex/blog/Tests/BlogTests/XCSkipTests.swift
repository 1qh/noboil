...
internal let isJava = ProcessInfo.processInfo.environment["java.io.tmpdir"] != nil
/// True when running within an Android environment (either an emulator or device)
internal let isAndroid = isJava && ProcessInfo.processInfo.environment["ANDROID_ROOT"] != nil
/// True is the transpiled code is currently running in the local Robolectric test environment
internal let isRobolectric = isJava && !isAndroid
/// True if the system's `Int` type is 32-bit.
internal let is32BitInteger = Int64(Int.max) == Int64(Int32.max)
