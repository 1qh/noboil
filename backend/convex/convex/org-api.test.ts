...
const createOrg = async (ctx: TestCtx, userId: string, slug: string, name?: string) =>
  ctx.run(async c =>
    c.db.insert('org', { name: name ?? `${slug} org`, slug, updatedAt: Date.now(), userId: userId as never })
  )
// eslint-disable-next-line @typescript-eslint/max-params
const addMember = async (ctx: TestCtx, orgId: string, userId: string, isAdmin = false) =>
  ctx.run(async c =>
...
describe('org management', () => {
  describe('org.create', () => {
    test('creates org with valid data', async () => {
      ...
      const result = await asUser(0).mutation(api.org.create, { data: { name: 'My Org', slug: 'my-org' } })
  ...
  describe('org.update', () => {
    test('admin can update org name', async () => {
      ...
      const orgId = await createOrg(ctx, ownerId, 'update-test')
    ...
    test('admin member can update org', async () => {
      ...
      const orgId = await createOrg(ctx, ownerId, 'admin-update')
    ...
    test('regular member cannot update org', async () => {
      ...
      const orgId = await createOrg(ctx, ownerId, 'member-update')
    ...
    test('rejects slug update to taken slug', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'slug-a')
    ...
    test('allows updating slug to own slug', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'same-slug')
  ...
  describe('org.get', () => {
    test('member can get org', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'get-test')
      await addMember(ctx, orgId, memberId)
    ...
    test('owner can get org', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'owner-get')
      const org = await asUser(0).query(api.org.get, { orgId })
      expect(org).not.toBeNull()
    })
    test('non-member cannot get org', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'no-access')
      let threw = false
      try {
  ...
  describe('org.getBySlug', () => {
    ...
    test('returns null for non-existent slug', async () => {
      const ctx = t()
      const org = await ctx.query(api.org.getBySlug, { slug: 'does-not-exist' })
      expect(org).toBeNull()
    })
  })
  describe('org.getPublic', () => {
    ...
    test('returns null for non-existent slug', async () => {
      const ctx = t()
      const org = await ctx.query(api.org.getPublic, { slug: 'nope-nope' })
      expect(org).toBeNull()
    })
  })
  describe('org.myOrgs', () => {
    ...
    test('lists member orgs with correct role', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'member-list')
      await addMember(ctx, orgId, memberId)
      const orgs = await asUser(1).query(api.org.myOrgs, {})
      const found = orgs.find((o: Record<string, unknown>) => (o.org as Record<string, unknown>).slug === 'member-list')
      expect(found).toBeDefined()
    ...
    test('admin member gets admin role', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'admin-role')
      await addMember(ctx, orgId, adminId, true)
      const orgs = await asUser(1).query(api.org.myOrgs, {})
      const found = orgs.find((o: Record<string, unknown>) => (o.org as Record<string, unknown>).slug === 'admin-role')
      expect(found).toBeDefined()
  ...
  describe('org.remove', () => {
    test('owner can remove org', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'remove-me')
    ...
    test('non-owner cannot remove org', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'no-remove')
    ...
    test('remove cascades members, invites, and join requests', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'cascade-rm')
      await addMember(ctx, orgId, memberId)
      ...
      const remaining = await ctx.run(async c => {
        ...
        const invs = await c.db.query('orgInvite').collect()
        const jrs = await c.db.query('orgJoinRequest').collect()
    ...
    test('remove cascades orgCascadeTables (task, project)', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'cascade-tables')
      await ctx.run(async c => {
      ...
      const remaining = await ctx.run(async c => {
        ...
        const ts = await c.db.query('task').collect()
  ...
  describe('org.isSlugAvailable', () => {
    test('returns available: true for unused slug', async () => {
      const ctx = t()
      const result = await ctx.query(api.org.isSlugAvailable, { slug: 'fresh-slug' })
...
describe('org members', () => {
  describe('org.membership', () => {
    test('returns owner role for org creator', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'membership-owner')
      const result = await asUser(0).query(api.org.membership, { orgId })
    ...
    test('returns member role', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'membership-member')
      await addMember(ctx, orgId, memberId)
    ...
    test('returns admin role', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'membership-admin')
      await addMember(ctx, orgId, adminId, true)
      const result = await asUser(1).query(api.org.membership, { orgId })
      expect(result).not.toBeNull()
    ...
    test('returns null for non-member', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'membership-none')
      const result = await asUser(1).query(api.org.membership, { orgId })
  ...
  describe('org.members', () => {
    test('lists owner and members', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'members-list')
      await addMember(ctx, orgId, memberId)
    ...
    test('non-member cannot list members', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'members-denied')
      let threw = false
      try {
  ...
  describe('org.setAdmin', () => {
    test('owner can promote member to admin', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'set-admin')
      const memberDocId = await addMember(ctx, orgId, memberId)
    ...
    test('owner can demote admin to member', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'demote-admin')
      const memberDocId = await addMember(ctx, orgId, adminId, true)
    ...
    test('non-owner cannot setAdmin', async () => {
      ...
      const [ownerId, adminId, thirdId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'set-admin-denied')
      await addMember(ctx, orgId, adminId, true)
  ...
  describe('org.removeMember', () => {
    test('admin can remove regular member', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'rm-member')
      const memberDocId = await addMember(ctx, orgId, memberId)
    ...
    test('cannot remove org owner', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'rm-owner-fail')
      const ownerMemberId = await addMember(ctx, orgId, ownerId)
      await addMember(ctx, orgId, adminId, true)
      let threw = false
      try {
    ...
    test('admin cannot remove another admin', async () => {
      ...
      const [ownerId, admin1Id, admin2Id] = userIds
      const orgId = await createOrg(ctx, ownerId, 'rm-admin-fail')
    ...
    test('owner can remove admin', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'owner-rm-admin')
      const adminDocId = await addMember(ctx, orgId, adminId, true)
  ...
  describe('org.leave', () => {
    test('member can leave org', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'leave-org')
      await addMember(ctx, orgId, memberId)
    ...
    test('owner cannot leave (must transfer first)', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'owner-leave')
      let threw = false
      try {
  ...
  describe('org.transferOwnership', () => {
    test('owner can transfer to admin', async () => {
      ...
      const [ownerId, adminId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'transfer-test')
      await addMember(ctx, orgId, adminId, true)
    ...
    test('non-owner cannot transfer', async () => {
      ...
      const [ownerId, adminId, thirdId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'transfer-deny')
      await addMember(ctx, orgId, adminId, true)
    ...
    test('cannot transfer to non-admin member', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'transfer-non-admin')
      await addMember(ctx, orgId, memberId)
      let threw = false
      try {
...
describe('org invites', () => {
  describe('org.invite', () => {
    test('admin can create invite', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'invite-org')
      const result = await asUser(0).mutation(api.org.invite, { email: 'invited@test.com', isAdmin: false, orgId })
    ...
    test('regular member cannot invite', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'invite-denied')
      await addMember(ctx, orgId, memberId)
      let threw = false
      try {
  ...
  describe('org.acceptInvite', () => {
    test('valid token adds user as member', async () => {
      ...
      const [ownerId, inviteeId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'accept-invite')
      const token = 'valid-accept-token'
      await ctx.run(async c => {
        await c.db.insert('orgInvite', {
    ...
    test('expired token is rejected', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'expired-invite')
    ...
    test('already member gets ALREADY_ORG_MEMBER', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'already-member')
      await addMember(ctx, orgId, memberId)
    ...
    test('invite with isAdmin: true creates admin member', async () => {
      ...
      const [ownerId, inviteeId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'admin-invite')
  ...
  describe('org.revokeInvite', () => {
    test('admin can revoke invite', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'revoke-invite')
      const inviteId = await ctx.run(async c =>
  ...
  describe('org.pendingInvites', () => {
    test('admin can list pending invites', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'pending-invites')
      await ctx.run(async c => {
        await c.db.insert('orgInvite', {
          email: 'a@test.com',
          expiresAt: Date.now() + 86_400_000,
...
describe('org join requests', () => {
  describe('org.requestJoin', () => {
    test('non-member can request to join', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'join-req')
      const result = await asUser(1).mutation(api.org.requestJoin, { message: 'Please let me in', orgId })
    ...
    test('existing member cannot request join', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'join-already-member')
      await addMember(ctx, orgId, memberId)
      let threw = false
      try {
    ...
    test('duplicate pending request is rejected', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'join-dup')
      await asUser(1).mutation(api.org.requestJoin, { orgId })
      let threw = false
      try {
        await asUser(1).mutation(api.org.requestJoin, { orgId })
  ...
  describe('org.approveJoinRequest', () => {
    test('admin can approve request (creates member)', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'approve-join')
      const requestId = await ctx.run(async c =>
    ...
    test('approve with isAdmin creates admin member', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'approve-admin')
      const requestId = await ctx.run(async c =>
        c.db.insert('orgJoinRequest', { orgId, status: 'pending', userId: requesterId })
      )
  ...
  describe('org.rejectJoinRequest', () => {
    test('admin can reject request', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'reject-join')
      const requestId = await ctx.run(async c =>
        c.db.insert('orgJoinRequest', { orgId, status: 'pending', userId: requesterId })
      )
  ...
  describe('org.cancelJoinRequest', () => {
    test('requester can cancel own pending request', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'cancel-join')
      const requestId = await ctx.run(async c =>
        c.db.insert('orgJoinRequest', { orgId, status: 'pending', userId: requesterId })
      )
    ...
    test('cannot cancel someone elses request', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'cancel-other')
      const requestId = await ctx.run(async c =>
        c.db.insert('orgJoinRequest', { orgId, status: 'pending', userId: requesterId })
      )
      let threw = false
    ...
    test('cannot cancel non-pending request', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'cancel-approved')
      const requestId = await ctx.run(async c =>
  ...
  describe('org.pendingJoinRequests', () => {
    test('admin can list pending requests', async () => {
      ...
      const [ownerId, req1Id, req2Id] = userIds
      const orgId = await createOrg(ctx, ownerId, 'pending-reqs')
      await ctx.run(async c => {
    ...
    test('regular member cannot list pending requests', async () => {
      ...
      const [ownerId, memberId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'pending-denied')
      await addMember(ctx, orgId, memberId)
      let threw = false
      try {
  ...
  describe('org.myJoinRequest', () => {
    test('returns pending request for current user', async () => {
      ...
      const [ownerId, requesterId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'my-join-req')
      await ctx.run(async c => {
    ...
    test('returns null when no pending request', async () => {
      ...
      const [ownerId] = userIds
      const orgId = await createOrg(ctx, ownerId, 'no-join-req')
      const result = await asUser(1).query(api.org.myJoinRequest, { orgId })
      expect(result).toBeNull()
    })
  })
